...
import { probeMultiConfigure } from './multiconfigure.js';
...
async function probeSeaDrop(chainKey, contract) {
  ...
  if (!allowed?.length) {
    ...
    if (!looksSeaDrop) return null;

    // A SeaDrop-family token with no readable registry pointer. Its live config
    // is unreachable from state, but the transaction that set it is public.
    const recovered = await probeMultiConfigure(chainKey, contract);
    ...
    return {
      ...
      findings: [],
      sources: [],
      warnings: ['This contract implements the SeaDrop minting interface, but does not expose getAllowedSeaDrop(), and no drop-configuration transaction was found in recent history. Its phase schedule is therefore not readable from public chain data - the mint platform renders it from its own API.'],
    };
  }
